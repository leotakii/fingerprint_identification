...
def main():#PATH TESTADO: 'Rindex28/'
	if len(sys.argv) != 3:
		print ("Utilize o seguinte formato de entrada: main_ljt_fingerprints.py [base] (\"L\" ou \"R\") [entrega] (\"1\" ou \"2\" OU \"0\" PARA NAO VISUALIZAR IMAGENS#)")
		return 1
	...
	if path == 'L':
		...
		print("Erro! Arg 1 (base de impressoes digitais) deve ser \"L\" ou \"R\"")
		return 1

	if entrega != '1' and entrega != '2' and entrega != '0':
		print("Erro! Arg 2 (parte do trabalho) deve ser \"1\" ou \"2\"\nCaso nao deseje visualizar imagens, o argumento deve ser \"0\"")
		return 1
	...
	print("Aprimoramento das imagens concluido")
	for image in processedImages: #CALCULOS DOS GRADIENTES DOS BLOCOS E ZONAS DE INTERESSE
		...
		print("Blocos de orientacao calculados")
		##SIR BENNA METHOD
		# Consistency level, filter of size (2*cons_lvl + 1) x (2*cons_lvl + 1)
		cons_lvl = 1  
		...
		print("Suavizacao dos blocos de orientacao")
		#time.sleep(100)
		...
		print("Representacao grafica dos blocos de orientacao")
		...
		print("Calculado os pontos de interesse da imagem")
		...
		for i in range(1, len(orientation_blocks_smooth[0]) - 1): 
			for j in range(1, len(orientation_blocks_smooth[1]) - 1):
				...
				if (np.isclose(poincare[i, j], 180, 0, tolerance)):
					singularity = "loop"
					draw.rectangle([(j * W, i * W), ((j + 1) * W, (i + 1) * W)],fill=(64,64,64,0), outline = colors[singularity])
		...
		tempComp = 	[]
		componentCores = []
		print("Blocos de singularidade obtidos")
...
##Processamento dos componentes deltas medianos
		...
		tempComp = []
		print("Blocos de singularidade separados em componentes")
		...
		for component in componentCores:
			...
			draw.ellipse([(centralCoord[1] * W, centralCoord[0] * W), ((centralCoord[1] + 1) * W, (centralCoord[0] + 1) * W)],fill=(128,128,128,0), outline = colors["loop"])
		...
		for component in componentDeltas:
			...
			draw.ellipse([(centralCoord[1] * W, centralCoord[0] * W), ((centralCoord[1] + 1) * W, (centralCoord[0] + 1) * W)],fill=(255,255,255,0), outline = colors["delta"])
		print (componentCores)
		
		print("Calculado o ponto medio de cada componente")
		del draw
		...
		print("Posicionamento dos deltas estabelecido")
		...
		print("Tipo de digital classificado: ",type)
		
		if entrega == "2" or entrega == "0":
			
			#################MINUNCIAS
			image_bin = cleaner.binarize(image)	
			print('Binarizacao Concluida')	
			preDilate = image_bin
			...
			print('Thinning Concluido')
			
			for index_x in drange(0,image.shape[0],subMatrixBlockSize): ##pinta de branco as zonas nao interessantes para visualizacao
				for index_y in drange(0,image.shape[1],subMatrixBlockSize):
			...
			image_spook = np.where(skeletonized < 255, 0, 1).astype('uint8')   
			...
			if entrega != "0":
				...
				plt.text(0, 0, type, fontsize=15)
				plt.imshow(cv2.resize(np.hstack((image,image_bin,image_smoothed,skeletonized)),None, fx=1.2, fy=1.2),cmap="gray")
				plt.show()
